...
export default function GlobalPlayer() {
  const {
    currentBeat, isPlaying, togglePlay, setPlaying, volume, setVolume, setProgress,
    skipNext, skipPrev, currentTime, duration, repeat, shuffle, cycleRepeat, toggleShuffle, closePlayer,
  } = usePlayerStore()
  ...
  useEffect(() => {
    ...
    wavesurfer.current.on('finish', () => {
      // Read live state from the store so repeat/shuffle/queue changes made
      // after this track loaded are always respected (no stale closure).
      const state = usePlayerStore.getState()
      if (state.repeat === 'one') {
        wavesurfer.current?.seekTo(0)
        wavesurfer.current?.play()
      } else {
        const advanced = state.skipNext()
        if (!advanced) state.setPlaying(false)
      }
    })
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Reflect the new position immediately even while paused (audioprocess
    // only fires during playback).
    const dur = wavesurfer.current?.getDuration() || duration
    if (dur > 0) setProgress(percent * dur, dur)
  }
          ...
          <div className="flex items-center gap-3 px-4 py-3" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <button
                ...
                className="w-[52px] h-[52px] bg-[#FF5500] rounded-full flex items-center justify-center shadow-[0_0_24px_rgba(255,85,0,0.5)] active:scale-95 transition-transform"
              >
                {isPlaying
        ...
        <div className="w-full max-w-5xl pointer-events-auto animate-slide-up">
          ...
          <div className="flex items-center flex-1 gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <button onClick={skipPrev} aria-label="Previous track" className="w-8 h-8 flex items-center justify-center text-text-muted hover:text-white transition-colors">
              ...
              <button onClick={skipNext} aria-label="Next track" className="w-8 h-8 flex items-center justify-center text-text-muted hover:text-white transition-colors">
